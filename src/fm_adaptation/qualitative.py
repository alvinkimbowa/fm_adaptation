...
def _panels(image, gt, prediction, layout, style):
    ...
    for slot in LAYOUTS[layout]:
        if slot == "image":
            ...
            panel[gt] = style["gt_color"]
        elif slot == "prediction_mask":
            panel = np.zeros_like(plain)
            panel[prediction] = style["pred_color"]
        else:
            panel = plain.copy()
            # Prediction first so a ground-truth contour stays legible on top of a filled overlay.
            if slot in ("both", "prediction"):
                draw(panel, prediction, style["pred_style"], style["pred_color"], style["pred_width"], style["alpha"])
            if slot in ("both", "gt"):
                draw(panel, gt, style["gt_style"], style["gt_color"], style["gt_width"], style["alpha"])
...
def select_cases(prediction_dir, count, metrics=None):
...
def render(images, labels, predictions, output, layout="pair", rows=3, cols=2, metrics=None,
           crop=None, seed=0, style=None, title=None):
    ...
    cases = select_cases(predictions, rows * cols, metrics)
    if not cases:
        return None
    ...
    for index, (case_id, dice) in enumerate(cases):
        ...
        panels = _panels(
            np.asarray(image), np.asarray(label) > 0, np.asarray(prediction) > 0, layout, style
        )
